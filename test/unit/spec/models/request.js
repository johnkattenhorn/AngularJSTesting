...
describe('Model: RequestModel', function () {

    var RequestModel, $httpBackend, $rootScope, APIBaseUrl, RequestRepository, collectionUrl = 'requests';

    beforeEach(function () {

        RequestRepository = jasmine.createSpy('RequestRepository');
        RequestRepository.attach = jasmine.createSpy('RequestRepository.attach');

        module('patradingwebApp', function ($provide) {
            $provide.value('RequestRepository', RequestRepository);
        });

        inject(function (_RequestModel_, _$httpBackend_, _$rootScope_, _APIBaseUrl_) {
    ...
    it('should have the url property set', function() {
        expect(RequestModel.$settings.url).toBe(APIBaseUrl + collectionUrl);
    });
    ...
    describe('$save', function () {
        it('should PUT its data on $save when it has an ID (update existing)', function() {
            ...
            var model = new RequestModel({title: 'New title', id: 5});
        ...
        it('should POST its data on $save if does not have an ID (new)', function() {
            ...
            var model = new RequestModel({title: 'New title'});
        ...
        it('should attach itself to the Repository on save', function() {
            $httpBackend.expectPUT( APIBaseUrl + collectionUrl + '/5', {title:'New title', id:5}).respond(200, {id: 5, title:'New title from server'});
            var model = new RequestModel({title: 'New title', id: 5});
            expect(RequestRepository.attach).not.toHaveBeenCalled();
            var promise = model.$save();
            $httpBackend.flush();
            expect(RequestRepository.attach).toHaveBeenCalledWith(model);
        });
    });
    ...
    describe('$set', function () {
        it('should load instance and override with new data', function() {
            ...
            expect(model instanceof RequestModel).toBeTruthy();
        });
    ...
    describe('$isDirty', function () {
        it('should return false if object is not changed since last save or delete ', function() {
            var model = new RequestModel({id:1});
            expect(model.$isDirty).toBeFalsy();
        });

        it('should not be dirty initially', function() {
            var model = new RequestModel({id:5});
            expect(model.$isDirty).toBeFalsy();
            $rootScope.$digest();
            expect(model.$isDirty).toBeFalsy();
        });
